...
int mpi_size, mpi_rank, mpi_other;
MPI_Request req;
...
int main(int argc, char* argv[])
{
    ...
    mpi_other = mpi_rank ^ 1;

    gettimeofday(&temp_time, NULL);
    //printf("R%d MPI_Init> %g s\n", mpi_rank, CAL_TIME);
    ...
    gettimeofday(&temp_time, NULL);
    //printf("R%d block_FW> %g s\n", mpi_rank, CAL_TIME);

    MPI_Finalize();

    gettimeofday(&temp_time, NULL);
    //printf("R%d MPI_Fin> %g s\n", mpi_rank, CAL_TIME);
...
void block_FW()
{
    ...
    for(int i=first_round; i<Rounds; i++) {
        ...
        ptr_d += line_d;
        p2_start += line_d;
        if(i==first_round || id_0[i]==mpi_rank) {
            if(flag>0) {
                if(id_0[i]!=mpi_rank) {
                    row = 1;
                    bline = block_size;
    ...
    for (int r=first_round; r<Rounds; ++r) {
        if(r>first_round) {
            if(id_0[r]==mpi_rank) {
                ...
                MPI_Send(ptr_h, n*bline, MPI_INT, mpi_other, 0, MPI_COMM_WORLD);
            } else {
        ...
        int next_r = r + 1;
        if(next_r<Rounds) {
            ptr_d = line_d * next_r + Dist_d;
            ptr_h = line_n * next_r + Dist_h;
            bline = next_r==Rounds-1 ? last_line : block_size;
            if(id_0[next_r]==mpi_rank) {
                ...
                MPI_Irecv(ptr_h, n*bline, MPI_INT, mpi_other, 0, MPI_COMM_WORLD, &req);
            }
        }

        flag = 1;
        for(int i = (r+1) % Rounds; i != r; i = (i==Rounds-1) ? 0 : i+1) {
            if((id_0[i]!=mpi_rank && r<Rounds-1) || i==r+1) continue;
            ...
            if(flag>0) {
                ...
                if(r==Rounds-1) {
                    if(w_once) {
                        //int offset = ptr_h - Dist_h;
                        //fprintf(stderr, "rank%d off%d i%d(%d) size%d\n", mpi_rank, offset, offset/line_n, id_0[offset/line_n], w_size);
                        if(mpi_rank==1) {
                            cudaEventSynchronize(ev_m);
                            MPI_Send(ptr_h, w_size, MPI_INT, mpi_other, 0, MPI_COMM_WORLD);
                        } else {
                            MPI_Recv(ptr_h, w_size, MPI_INT, mpi_other, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                        }
                    }
...
void input(char *outFileName)
{
    ...
    if(mpi_rank==0) {
        ...
        cudaMallocHost(&Dist_h, out_size);
    }
    ...
    gettimeofday(&temp_time, NULL);
    //printf("R%d\tcuda allocate done> %g s\n", mpi_rank, CAL_TIME);
}
...
void block_FW_S()
{
    ...
    for(int i=1; i<Rounds; i++) {
        ...
        if(i>1) {
            if(dev==mpi_rank) {
                cudaEventSynchronize(ev_m);
                MPI_Isend(ptr_h, n*bline, MPI_INT, mpi_other, 0, MPI_COMM_WORLD, &req);
            }
            MPI_Wait(&req, MPI_STATUS_IGNORE);
            if(dev!=mpi_rank) {
                cudaMemcpy2DAsync(ptr_d, pitch_bytes, ptr_h, n_bytes, n_bytes, bline, cudaMemcpyHostToDevice, stream_m);
        ...
        for(int j=0; j<Rounds; j++) {
            ...
            if(j==i+1) {
                ptr_d = j * line_d + Dist_d;
                ptr_h = j * line_n + Dist_h;
                bline = j==Rounds-1 ? last_line : block_size;
                if(dev==mpi_rank) {
                    cudaEventRecord(ev_m, stream[j]);
                    cudaStreamWaitEvent(stream_m, ev_m, 0);
                    cudaMemcpy2DAsync(ptr_h, n_bytes, ptr_d, pitch_bytes, n_bytes, bline, cudaMemcpyDeviceToHost, stream_m);
                    cudaEventRecord(ev_m, stream_m);
                } else {
    ...
    for(int i=0; i<Rounds; i++) {
        ...
        if(i%2==mpi_rank) {
            ...
            if(mpi_rank==1) {
                fprintf(stderr, "Rank 1 send %d\n", i);
                MPI_Send(ptr_h, n*bline, MPI_INT, 0, 0, MPI_COMM_WORLD);
            }
        } else if(mpi_rank==0) {
            fprintf(stderr, "Rank 0 recv %d\n", i);
            MPI_Recv(ptr_h, n*bline, MPI_INT, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
        ptr_h += line_n;
        ptr_d += line_d;
    }
